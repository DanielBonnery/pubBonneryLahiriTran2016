...
plot1<-ggplot(data = xys, aes(x = ftemp07, y = ftemp12)) +
...
plot2bisbis<-ggplot(data = xy, aes(x = lftemp07, y = lftemp12)) + 
  ...
  #geom_smooth(color="yellow",se=FALSE) +geom_abline(slope=1,intercept=0,color="red")+
  ...
  guides(alpha=FALSE) + #  
facet_wrap(~type_of_gov)
save(plot2bisbis,file="figure/plot2bisbis.rda")
